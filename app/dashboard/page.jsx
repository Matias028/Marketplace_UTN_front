...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const fetchUserCars = async () => {
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/car/user/${parsedUser.id}`, {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/car/${carToDelete.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/car/${carToEdit.id}`, {
        method: "PUT",
        headers: {
